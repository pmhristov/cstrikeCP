<?php 
	...
	$get_amx_plugins = $DB->GetAll('SELECT * FROM `plugins` WHERE `server_id` = ?', array($_SESSION['id']));
	$last_plugins = $DB->GetAll('SELECT * FROM `pluginlist` WHERE `hidden` = ? ORDER BY `id` DESC LIMIT 8', array(0));
?>
		...
		<div class="row-fluid">
			<div class="span12">
					<div class="alert alert-info alert-block">
						...
						Желаете да направите вашия сървър уникален по рода си ? Тука е мястото от където можете да добавите различни екстри, защити и допълнения към него.<br />
						Можете да проверявате статуса на вашите добавки чрез командата <strong>amx_plugins</strong>
					</div>					
