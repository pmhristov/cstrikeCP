...
class EasyPayPayment extends Payment {
	...
	public function process() {
		...
		$lines = explode("\n", $decoded);
 		...
 		foreach ($lines as $line) {
			if (preg_match('/^INVOICE=(?P<invoice>\d+):STATUS=(?P<status>PAID|DENIED|EXPIRED)(?P<extra>:PAY_TIME=(?P<pay_time>\d+):STAN=(?P<stan>\d+):BCODE=(?P<bcode>[0-9a-zA-Z]+))?$/', $line, $details)) {
                $order = $DB->GetRow('SELECT * FROM `orders` WHERE `epay_invoice_id` = ?', array($details['invoice']));
				...
				try {
					switch ($details['status']) {
						case 'PAID':
							$DB->AutoExecute('orders', array('paid' => 1), 'UPDATE', '`id` = ' . $DB->qstr($order['id']));
						break;
						case 'DENIED':
						case 'EXPIRED':
							$DB->Execute('DELETE FROM `orders` WHERE `id` = ?', array($order['id']));
						break;
					}
		...
		file_put_contents('/var/www/vhosts/www.smshosting.bg/easypay.txt', $decoded . PHP_EOL . $response . str_repeat('=', 80) . PHP_EOL, FILE_APPEND);
	}
